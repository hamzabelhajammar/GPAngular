...
export class UserService {
  ...
  retrieveAllUsersAndIdBox(email: string): Observable<any> {
    return this.http.get(`${this.URL1}/retrieveAllUsersAndIdBox/${email}`);
  }
  ...
  searchUsers(value: string): Observable<User[]> {
    const url = `http://localhost:8075/user/searchUsers/${value}`;
    return this.http.get<User[]>(url);
  }
  ...
  saveToken(token: string,email:string): Observable<User> {
    const url = `${this.URL1}/saveToken/${email}/${token}`;
    return this.http.get<User>(url);
  }
